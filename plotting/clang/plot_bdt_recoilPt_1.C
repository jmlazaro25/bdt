...
void drawRatio(
    ...
    h1->Draw("axis");
    TLegend* leg = new TLegend(0.45, legymin, 0.65, legymax);
...
void plot_bdt_recoilPt_1(
    ...
    proclabel["nonfiducial"] = "Nonfiducial Events";
    proclabel["trigger"] = "Trigger Selected Fiducial Events";
    proclabel[bdtName1] = bdtName1 + " #varepsilon(bkg) = " + effstr;
    // proclabel[bdtName1] = "Gabrielle #varepsilon(bkg) = " + effstr;
    proclabel[bdtName2] = bdtName2 + " #varepsilon(bkg) = " + effstr;
    // proclabel[bdtName1] = "seg-mipv3 #varepsilon(bkg) = " + effstr;
    proclabel[bdtName3] = bdtName3 + " #varepsilon(bkg) = " + effstr;
    // proclabel[bdtName1] = "seg-mipv4 #varepsilon(bkg) = " + effstr;

    // Set up variable bin widths
    int n_bins = 14;
    ...
    colors[bdtName1] = color_comp5;
    colors[bdtName2] = color_comp2;
    colors[bdtName3] = color_comp3;

    // Define the selections we want to apply
    vector<TString> sels = {withSelect, bdtName1, bdtName2, bdtName3};
    ...
    for(auto seln : sels) {
        if (seln == bdtName1) {
        ...
        else if(seln == bdtName3) {
            TH1D* bkghist = 0;

            // Read tree from input file
            TFile* file = new TFile(flatDir3 + "/bkg_" + bdtName3 + "_eval.root");
            TTree* tree = (TTree*)file->Get("EcalVeto");
            assert(tree);
            ...
            // Read off disc values from tree into histogram
            tree->Draw("discValue_" + bdtName3 + ">>" + seln + "_bkg", sel[withSelect]);
            ...
            // Get cut value for selected background efficiency
            float cutval = getCutValueForEfficiency(bkghist, bkgEff)[0];
            cout << "\nCut value for BDT version (" + bdtName3 + "): cutval = " << cutval;
            sel[seln] = sel[withSelect] + " && discValue_" + bdtName3 + " > " + cutval;
        }
    ...
    for(auto proc : procs) {
        ...
        for(auto seln : sels) {
            ...
            if(seln == bdtName1) {
                ...
                cout << "Getting data for BDT version (" + bdtName3 + ")..." << endl;
                file = new TFile(flatDir3 + "/" + proc + "_" + bdtName3 + "_eval.root");
                tree = (TTree*)file->Get("EcalVeto");
            }
	    ...
	    hist->SetBins(n_bins, xbins_dbl);
            ...
            // Make the plots using the data from the file
        ...
        // Draw histograms for all processes on the canvas
        for(auto* hist : hists) {
        ...
        if(proc == "0.001")
            ...
            drawRatio(c, hists, labels, 0.85, 0.65, 0.0025, 0.00005, true);
        ...
        // Save the plot
        c->SaveAs(outDir + "/" + withSelect + "_" + bdtName1 + "_" + bdtName2 + "_" + bdtName3 + "_" + proc + "_recoilPt.pdf");
    }
}
