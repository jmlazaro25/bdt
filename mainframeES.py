...
def main():
    ...
    print('\n' + f'Using {action_str} action from conf file: {configFile}')
...
class BdtTrainer():
    ...
    def run(self, strEvent=None, maxEvents=1.25e6 , pfreq=None):
        ...
        for st in self.sets:
            ...
            new_ind = np.random.permutation(
                    np.arange( np.shape(events)[0] )
                    )
            np.take(events, new_ind, axis = 0, out=events)

            train_frac = 0.8 # Not intended for perm hard code, but no promise
            setattr(self, f'{st}_train_x',events[:int(train_frac*len(events))])
            setattr(self, f'{st}_test_x',events[int(train_frac*len(events)):])
            setattr(self,
                    f'{st}_train_y',
                    np.zeros(
                        len(
                            getattr( self, f'{st}_train_x')
                            )
                        ) + (st == 'sig')
                    )
            setattr(self,
        ...
        test_x  = np.vstack(( self.sig_test_x, self.bkg_test_x ))
        test_y  = np.append( self.sig_test_y, self.bkg_test_y )
        training_matrix = xgb.DMatrix(train_x, train_y,
                weight=np.append(
                    np.ones(len(self.sig_train_y)),
                    np.ones(len(self.bkg_train_y)) \
                            * len(self.sig_train_y)/len(self.bkg_train_y)
                    )
                )
        testing_matrix = xgb.DMatrix(test_x, test_y)

        # Actual training
        evallist = ( (testing_matrix,'eval'), (training_matrix,'train') )
        gbm = xgb.train(
                self.params_dict,
                training_matrix,
        ...
        output = open(f'{self.outdir}/{outname}_weights.pkl', 'wb')
        pkl.dump(gbm, output)

        # Some metrics 'n' stuff
        preds = gbm.predict(testing_matrix)
        fpr, tpr, threshold = metrics.roc_curve(test_y, preds)
        roc_auc = metrics.auc(fpr, tpr)
        print(f'AUC = {roc_auc}')
        np.savetxt(f'{self.outdir}/{outname}_validation_preds.txt',preds)
        np.savetxt(f'{self.outdir}/{outname}_validation_threetuples.txt',
                np.c_[fpr,tpr,threshold]
                )
                                                                                 
        # Plot feature importances
        xgb.plot_importance(gbm)
        plt.pyplot.savefig(
            f'{self.outdir}/{outname}_fimportance.png', # png name
            dpi=500, bbox_inches='tight', pad_inches=0.5 # png parameters
            )
        
        # Anounce save location
        print(f'Files saved in: {self.outdir}')
